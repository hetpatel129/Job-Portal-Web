...
function Applicantstable() {
  ...
  const scheduleInterview = async () => {
    if (!interviewForm.interviewDate) return toast.error("Please select a date");
    try {
          ...
          <div key={a._id} className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm flex flex-col overflow-hidden hover:shadow-md transition-shadow">
            ...
            <div className="px-5 mb-3">
              {a.applicant.profile?.resume ? (
                <a href={import.meta.env.VITE_BASEURI + "user/resume/view?url=" + encodeURIComponent(a.applicant.profile.resume)} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 text-xs text-indigo-600 dark:text-indigo-400 hover:underline font-medium">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl w-full max-w-md p-6">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input type="datetime-local" value={interviewForm.interviewDate} onChange={(e) => setInterviewForm({ ...interviewForm, interviewDate: e.target.value })} className="w-full p-2.5 rounded-xl border border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-white text-sm focus:outline-none focus:ring-2 focus:ring-purple-500" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Location / Place</label>
                <input type="text" value={interviewForm.interviewPlace} onChange={(e) => setInterviewForm({ ...interviewForm, interviewPlace: e.target.value })} placeholder="e.g. Office Room 3, Google Meet..." className="w-full p-2.5 rounded-xl border border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-white text-sm focus:outline-none focus:ring-2 focus:ring-purple-500 placeholder-gray-400" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Note (optional)</label>
                <textarea value={interviewForm.interviewNote} onChange={(e) => setInterviewForm({ ...interviewForm, interviewNote: e.target.value })} placeholder="e.g. Bring portfolio..." rows={3} className="w-full p-2.5 rounded-xl border border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-white text-sm focus:outline-none focus:ring-2 focus:ring-purple-500 placeholder-gray-400 resize-none" />
              </div>
            </div>
            <div className="flex gap-3 mt-5">
              ...
              <button onClick={scheduleInterview} className="flex-1 py-2.5 rounded-xl bg-purple-600 hover:bg-purple-700 text-white text-sm font-semibold flex items-center justify-center gap-2 transition"><CheckCircle size={15} /> Confirm</button>
            </div>
          </div>
        </div>
      )}
